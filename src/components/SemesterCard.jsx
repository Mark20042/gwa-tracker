            ...
            <CardHeader className="pb-2 flex flex-row items-center justify-between">
                <div>
                    <CardTitle className="text-sm md:text-lg font-bold text-foreground flex items-center gap-2">
                        <span className="text-primary px-3 py-1 truncate max-w-[150px] md:max-w-none">{semester.term}</span>
                    </CardTitle>
                </div>
            ...
            <CardContent>
                ...
                <div className="mb-4 overflow-x-auto -mx-4 md:mx-0">
                    <Table className="min-w-full">
                        <TableHeader>
                            <TableRow>
                                <TableHead className="w-[40%] pl-4 md:pl-2">Subject Name</TableHead>
                                <TableHead className="w-[15%] text-center px-1">Units</TableHead>
                                <TableHead className="w-[15%] text-center px-1">Grade</TableHead>
                                <TableHead className="hidden sm:table-cell w-[15%] text-center">Status</TableHead>
                                    ...
                                    return (
                                        <TableRow key={sub.id} className="group">
                                            <TableCell className="font-medium text-xs py-2 pl-4 md:pl-2">
                                                <div className="truncate max-w-[120px] sm:max-w-none" title={sub.name || sub.code}>
                                                    {sub.name || sub.code}
                                                </div>
                                            </TableCell>
                                            ...
                                            <TableCell className="text-right py-2 pr-4 md:pr-2">
                                                <div className="flex justify-end gap-1">
                                                    <EditSubjectDialog yearId={yearId} semesterId={semester.id} subject={sub} />
                                                    <Button
                                                        variant="ghost"
                                                        size="icon"
